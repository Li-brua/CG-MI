...
import torch
from attacks.metrics.distance_metrics import DistanceEvaluation
from attacks.models.classifier import Classifier

import wandb
from attacks.utils.training_config_parser import TrainingConfigParser
...
def init_wandb_logging(target_model_name, config, args):
    # lr = optimizer.param_groups[0]['lr']
    # optimizer_name = type(optimizer).__name__
    # if not 'name' in config.wandb['wandb_nit_args']:
...
def log_nearest_neighbors(imgs, targets, eval_model, model_name, dataset, img_size, seed):
    ...
    evaluater = DistanceEvaluation(
        eval_model, None, None, img_size, None, dataset, seed)
